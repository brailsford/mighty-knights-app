...
const makeRoster = (count = 14) => Array.from({length: count}, (_, i) => ({ id: `p${i+1}`, name: `${i+1}. Player` }))

export default function MatchConsole() {
  const [dark, setDark] = useState(false)
  const [teamName, setTeamName] = useState('Mighty Knights')
  ...
  useEffect(() => {
    const starters = players.slice(0, maxOnField).map(p => ({ playerId: p.id, startMs: 0 }))
    setIntervals(starters)
    setEvents([{ type: 'START', atMs: 0 }])
    setMatchMs(0)
    setIsRunning(false)
    setSelectedBenchIds([])
    setSelectedOnIds([])
  ...
  const toggleClock = () => {
    snapshot()
    if (isRunning) { setIsRunning(false); setEvents(e => [...e, { type: 'PAUSE', atMs: matchMs }]) }
    else { setIsRunning(true); setEvents(e => [...e, { type: 'RESUME', atMs: matchMs }]) }
  }
  ...
  const quickAction = (kind, playerId) => {
    setEvents(e => [...e, { type: kind, atMs: matchMs, playerId, note: kind === 'OTHER' ? (noteDraft || undefined) : undefined }])
      ...
      <div className="space-y-4">
        <div className="mb-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={undo} className="rounded-xl border px-3 py-2 text-sm shadow">Undo</button>
          </div>
        </div>
          ...
          <div className="rounded-3xl border p-4 shadow md:col-span-2">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-gray-500">Half length: {halfLengthMin}′ • On field: {onField.length}/{maxOnField}</div>
              </div>
              <div className="flex gap-2">
                <button onClick={toggleClock} className={`rounded-2xl px-4 py-3 text-white shadow ${isRunning ? 'bg-red-600' : 'bg-green-600'}`}>{isRunning ? 'Pause' : 'Start'}</button>
              </div>
            </div>
          </div>
          <div className="rounded-3xl border p-4 shadow">
            <div className="mb-2 text-sm font-semibold">Match Settings</div>
            <label className="mb-2 block text-sm">Team size (a-side)
            ...
            <label className="mb-2 block text-sm">Half length
            ...
            <label className="mb-2 block text-sm">Opponent
              <input value={opponent} onChange={(e)=>setOpponent(e.target.value)} className="mt-1 w-full rounded-xl border px-3 py-2"/>
            </label>
            <label className="mb-2 block text-sm">Squad size
              <select onChange={(e)=> setPlayers(makeRoster(parseInt(e.target.value)))} className="mt-1 w-full rounded-xl border px-3 py-2" defaultValue={players.length}>
                {[10,12,14,16,18].map(v=> <option key={v} value={v}>{v} players</option>)}
              </select>
            </label>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          <div className="rounded-3xl border p-4 shadow lg:col-span-2">
            <div className="mb-2 flex items-center justify-between">
              ...
              {selectedBenchIds.length > 0 && <span className="text-xs text-blue-600">Select {selectedBenchIds.length} to sub OFF</span>}
            </div>
                  ...
                  <li key={p.id} className={`rounded-2xl border px-3 py-2 ${selected ? 'ring-2 ring-blue-500' : ''}`}>
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className={`mt-1 inline-block rounded-full px-2 py-0.5 text-[10px] tabular-nums ${ diff >= 60000 ? 'bg-red-50 text-red-700' : diff <= -60000 ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600' }`}>
                          {(diff >= 0 ? '+' : '-') + fmt(Math.abs(diff))}
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-1">
                        <button onClick={() => !locked && setSelectedOnIds(ids => ids.includes(p.id) ? ids.filter(x=>x!==p.id) : [...ids, p.id])} className={`rounded-xl px-3 py-1 text-sm ${locked ? 'bg-gray-200 text-gray-400' : 'bg-gray-100'}`} title={locked ? 'Locked until halftime' : 'Mark to sub OFF'}>
                        ...
                        <div className="flex gap-1">
                          ...
                          <button onClick={()=>{ setNoteDraft(p.name+': '); quickAction('OTHER', p.id) }} className="rounded-lg bg-gray-800 text-white px-2 py-1 text-[11px]">Other</button>
                        </div>
                        <label className="mt-1 flex items-center gap-1 text-[11px] text-gray-600">
                          <input type="checkbox" checked={!!lockUntilHT[p.id]} onChange={e => setLockUntilHT(prev => ({...prev, [p.id]: e.target.checked}))} /> Lock until HT
                        </label>
                      </div>
                    </div>
                  </li>
          ...
          <div className="rounded-3xl border p-4 shadow">
            <div className="mb-2 flex items-center justify-between">
              ...
              {selectedOnIds.length > 0 && <span className="text-xs text-blue-600">Select {selectedOnIds.length} to come ON</span>}
            </div>
            <ul className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              {bench.map(p => {
                ...
                return (
                  <li key={p.id}>
                    <div className={`w-full rounded-2xl border px-3 py-2 text-left shadow ${selected ? 'ring-2 ring-blue-500' : ''}`}>
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="font-medium">{p.name}</div>
                          <div className="text-xs text-gray-500">⏱ {fmt(played)} • tgt {fmt(fairShareMs)}</div>
                        ...
                        <button onClick={() => !heldForHT && setSelectedBenchIds(ids => ids.includes(p.id) ? ids.filter(x=>x!==p.id) : [...ids, p.id])} className={`rounded-xl px-3 py-1 text-sm ${heldForHT ? 'bg-gray-200 text-gray-400' : 'bg-gray-100'}`} title={heldForHT ? 'Will start at halftime' : 'Mark to come ON'}>
                          {heldForHT ? 'Hold for HT' : (selected ? 'Selected' : 'Mark ON')}
                        </button>
                      </div>
                      <label className="mt-1 flex items-center gap-1 text-[11px] text-gray-600">
                        <input type="checkbox" checked={!!startAtHT[p.id]} onChange={e => setStartAtHT(prev => ({...prev, [p.id]: e.target.checked}))} /> Start at HT
                      </label>
                    </div>
                  </li>
                )
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button onClick={suggestNow} className="rounded-xl px-3 py-2 text-sm shadow bg-blue-50 text-blue-700 border border-blue-200">Suggest rotation (now)</button>
              <button onClick={doBatchSub} disabled={!selectedBenchIds.length || !selectedOnIds.length || selectedBenchIds.length !== selectedOnIds.length} className={`rounded-xl px-3 py-2 text-sm shadow ${selectedBenchIds.length>0 && selectedOnIds.length>0 && selectedBenchIds.length===selectedOnIds.length ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-400'}`}>Confirm batch ({selectedBenchIds.length})</button>
              <button onClick={()=>{ setSelectedBenchIds([]); setSelectedOnIds([]); }} className="rounded-xl border px-3 py-2 text-sm shadow">Clear</button>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          <div className="rounded-3xl border p-4 shadow lg:col-span-2">
            <h3 className="mb-2 text-lg font-semibold">Minutes — now {fmt(fairShareMs)} • full-time {fmt(fullTimeTargetMs)}</h3>
              ...
              {players.map(p => {
                ...
                return (
                  <div key={p.id} className="rounded-2xl border px-3 py-2">
                    ...
                    <div className={`mt-1 inline-block rounded-full px-2 py-0.5 text-[10px] tabular-nums ${ diffNow >= 60000 ? 'bg-red-50 text-red-700' : diffNow <= -60000 ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600' }`}>now {(diffNow >= 0 ? '+' : '-') + fmt(Math.abs(diffNow))}</div>
                    <div className={`mt-1 inline-block rounded-full px-2 py-0.5 text-[10px] tabular-nums ${ diffFT >= 60000 ? 'bg-orange-50 text-orange-700' : diffFT <= -60000 ? 'bg-teal-50 text-teal-700' : 'bg-gray-100 text-gray-600' }`}>FT {(diffFT >= 0 ? '+' : '-') + fmt(Math.abs(diffFT))}</div>
                  </div>
                )
              })}
            </div>
          ...
          <div className="rounded-3xl border p-4 shadow">
            ...
            <ul className="space-y-2 max-h-80 overflow-auto pr-1">
              {events.map((ev, idx) => (
                <li key={idx} className="rounded-xl border px-3 py-2 text-sm flex items-center justify-between">
                  <span>
                    {ev.type === 'SUB_BATCH' ? (
                      <>
                        <span className="inline-block rounded bg-blue-50 px-2 py-0.5 text-blue-700 mr-2 text-xs">SUB×{(ev.pairs?.length ?? 0)}</span>
                        {ev.pairs?.map((p, i)=> <span key={i} className="mr-1">{p.onId}↔{p.offId}</span>)}
                      </>
                    ) : (ev.type === 'TRY' || ev.type === 'TACKLE' || ev.type === 'OTHER') ? (
                      <>
                        <span className="inline-block rounded bg-emerald-50 px-2 py-0.5 text-emerald-700 mr-2 text-xs">{ev.type}</span>
                        {ev.playerId ? `${ev.playerId}` : '(no player)'}
                        {ev.note ? ` — ${ev.note}` : ''}
                      </>
                    ) : (
                      <>
                        <span className="inline-block rounded bg-gray-50 px-2 py-0.5 text-gray-700 mr-2 text-xs">{ev.type}</span>
                        {ev.type === 'START' ? 'Match created' : ev.type === 'PAUSE' ? 'Clock paused' : 'Clock resumed'}
                      </>
                    )}
                  </span>
